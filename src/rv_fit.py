...
data_params = []
names = name.extract_star_names()

count = 0

for name in names:
    try:
        file_path = os.getcwd() + "\\star_data\\" + name + ".csv"

        star1 = Star(file_path, name)
        t = star1.get_t()
        rv = star1.get_rv()
        rv_err = star1.get_rv_err()

        star1.plot_rv(t, rv, rv_err, '.k')
        freq, power = star1.compute_periodiogram(t, rv, rv_err)
        star1.plot_periodogram(freq, power, 'k')

        per = star1.compute_period(freq, power)

        phase, k, voffset = star1.create_fit(per, freq, power, t, rv, rv_err)

        guess = star1.create_initial_params(
            k, per, 0.0, 0., t[np.argmax(rv)], voffset)
        params = star1.radial_vel_fit(t, rv, rv_err, guess)
        star1.plot_rvc_fit(phase, rv, rv_err, params)

        data_params.append(params)
        print("Star: {} added".format(name))

    except:
        pass


data = pd.DataFrame(data=data_params, columns=[
                    "Name", "K", "P", "e", "w", "tau", "n"])

data.to_csv(os.getcwd() + "\\params.csv")
